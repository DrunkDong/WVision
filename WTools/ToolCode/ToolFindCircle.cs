...
{
    [Serializable]
    public class ToolFindCircleParam : ToolParamBase
    {
        ...
        public ToolFindCircleParam()
        {
            mStepInfo = new StepInfo();
            mStepJumpInfo = new JumpInfo();
            StepInfo.mToolType = ToolType.FindCircle;
            ToolType = ToolType.FindCircle;
            ShowName = "查找圆";
            ToolName = "查找圆";
            mStepInfo.mShowName = "查找圆";
            mResultString = "";
            mImageSourceStep = -1;
            mImageSourceMark = -1;
            mLineSourceStep = -1;
    ...
    public class ToolFindCircle : ToolBase
    {
        ...
        public ToolFindCircle(ToolParamBase toolParam)
        {
            mToolParam = toolParam as ToolFindCircleParam;
            BindDelegate(true);
        }
        ...
        public override ResStatus BindDelegate(bool IsBind)
        {
            if (IsBind)
            {
                mToolParam.mParamChangedDe = new ToolFindCircleParam.ParamChangedDe(ParamChanged);
            }
            else
            {
        ...
        public override int DebugRun(HObject objj1, Bitmap objj2, List<StepInfo> StepInfoList, bool ShowObj, out JumpInfo StepJumpInfo)
        {
            ...
            if (mToolParam.mLineSourceStep < 0)
            {
                ...
                {
                    mToolParam.ResultString = "内部圆为空，请输入或绘制一条圆";
                    return 1;
                }
            }
            else
            {
                ...
                {
                    mToolParam.ResultString = "外部输入圆为空";
                    return 1;
                }
            }
            ...
            try
            {
                ...
                double rowFinal1, columnFinal1, radiusFinal1;
                //获取圆输入来源
                if (mToolParam.mLineSourceStep < 0)
                {
                    row1 = mToolParam.mRoiValue[0];
                    column1 = mToolParam.mRoiValue[1];
                    radius1 = mToolParam.mRoiValue[2];
                }
                else
                {
                    ...
                    radius1 = StepInfoList[mToolParam.mLineSourceStep - 1].mToolRunResul.mParamOutPut[2];
                }
                ...
                HOperatorSet.AddMetrologyObjectCircleMeasure(hv_MetrologyHandle, rowFinal1, columnFinal1, radiusFinal1,//参数导入模型
                    mToolParam.mMeasureHeight, mToolParam.mMeasureWidth, 0.5, mToolParam.mMeasureThreshold, param, paramValue, out hv_Index);
        ...
        public override int ToolRun(HObject objj1, Bitmap objj2, List<StepInfo> StepInfoList, bool ShowObj, out JumpInfo StepJumpInfo)
        {
            StepJumpInfo = new JumpInfo();
            mToolParam.ResultString = "";
            if (mToolParam.ForceOK)
