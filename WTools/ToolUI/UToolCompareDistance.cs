...
{
    [ToolboxItem(false)]
    public partial class UToolCompareDistance : UserControl,InterfaceUIBase
    {
        public UToolCompareDistance()
        {
            InitializeComponent();
        }
        ...
        public ResStatus InitRoiShow()
        {
            ...
            for (int i = 0; i < StepInfoList.Count; i++)
            {
                if (StepIndex < (i + 1))
                    break;
                if (StepInfoList[i].mToolType == ToolType.DistanceLL)
                {
        ...
        private void UToolCompareDistance_Load(object sender, EventArgs e)
        {
            mToolMachine = ToolMachine.GetInstance();
            mIsInit = false;
            InitParam();
