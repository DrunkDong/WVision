...
{
    [ToolboxItem(false)]
    public partial class UToolDistancePL : UserControl,InterfaceUIBase
    {
        public UToolDistancePL()
        {
            InitializeComponent();
        }
        ...
        public ResStatus InitRoiShow()
        {
            ...
            for (int i = 0; i < StepInfoList.Count; i++)
            {
                if (StepIndex < (i + 1))
                    break;
                if (StepInfoList[i].mToolResultType == ToolResultType.Circle|| StepInfoList[i].mToolResultType == ToolResultType.Point)
                {
        ...
        private void UToolDistancePL_Load(object sender, EventArgs e)
        {
            mToolMachine = ToolMachine.GetInstance();
            mIsInit = false;
            InitParam();
