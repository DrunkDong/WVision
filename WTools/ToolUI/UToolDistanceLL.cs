...
{
    [ToolboxItem(false)]
    public partial class UToolDistanceLL : UserControl,InterfaceUIBase
    {
        public UToolDistanceLL()
        {
            InitializeComponent();
        }
        ...
        public ResStatus InitRoiShow()
        {
            ...
            for (int i = 0; i < StepInfoList.Count; i++)
            {
                if (StepIndex < (i + 1))
                    break;
                if (StepInfoList[i].mToolType == ToolType.FindLine)
                {
        ...
        private void UToolDistanceLL_Load(object sender, EventArgs e)
        {
            mToolMachine = ToolMachine.GetInstance();
            mIsInit = false;
            InitParam();
